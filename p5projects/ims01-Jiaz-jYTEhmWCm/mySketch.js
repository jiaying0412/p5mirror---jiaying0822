//https://editor.p5js.org/jiaying0822/sketches/jYTEhmWCm
//ims01-Jiaz

//Attribution: “230107 Weird Faces” by Che-Yu Wuhttp://openprocessing.org/sketch/1785910License CreativeCommons Attribution NonCommercial ShareAlikehttps://creativecommons.org/licenses/by-nc-sa/3.0

//Link to sketch in OpenProcessing: <iframe src="https://openprocessing.org/sketch/1785910/embed/" width="400" height="400"></iframe>

//map(a=>"#"+a), for each element (a) in the array, add '#' to the beginning - jiaz

let colors1 = "ed1c24-fdfffc-235789-f1d302-020100"
  .split("-")
  .map((a) => "#" + a);
let colors2 = "2b2d42-8d99ae-edf2f4-ef233c-d80032"
  .split("-")
  .map((a) => "#" + a);
let colors3 = "d6fff6-231651-4dccbd-2374ab-ff8484"
  .split("-")
  .map((a) => "#" + a);
let colors4 = "a9e5bb-fcf6b1-f7b32b-f72c25-2d1e2f"
  .split("-")
  .map((a) => "#" + a);
let colors5 = "ff8811-f4d06f-fff8f0-9dd9d2-392f5a"
  .split("-")
  .map((a) => "#" + a);
let colors6 = "c5d86d-261c15-f7f7f2-e4e6c3-f05d23"
  .split("-")
  .map((a) => "#" + a);
let colors7 = "ff6700-ebebeb-c0c0c0-3a6ea5-004e98"
  .split("-")
  .map((a) => "#" + a);
let colors8 = "020202-0d2818-04471c-058c42-16db65"
  .split("-")
  .map((a) => "#" + a);
let colors9 = "003049-d62828-f77f00-fcbf49-eae2b7"
  .split("-")
  .map((a) => "#" + a);
let colors10 = "00072d-001c55-0a2472-0e6ba8-a6e1fa"
  .split("-")
  .map((a) => "#" + a);
let colors11 = "6b2d5c-f0386b-ff5376-f8c0c8-e2c290"
  .split("-")
  .map((a) => "#" + a);
let colors12 = "fb8b24-d90368-820263-291720-04a777"
  .split("-")
  .map((a) => "#" + a);
let _mouseX = 300,
  _mouseY = 300;
...
function mousePressed() {
  auto = false;
  playOscillator(); //haven't used the oscillator function yet, allows play with frequencies and amplitudes  - jiaz
  // 	// initParams()
}
...
// randomizes the faces on the screen - jiaz
function initParams() {
  colors = random([
    colors1,
    colors2,
...
//push() and pop() are functions that work together to save and restore the drawing style and transformations of your sketch. This is particularly useful when you want to apply certain transformations or styles to only a part of your sketch without affecting the rest, in this case, these functions are used to shift parts of the face back and forth before and after mouse press - jiaz
...
function drawEye(x, y, lr = 0) {
  ...
  rect(
    0,
    -80 + sin(lr + frameCount / 100 + _mouseX / 50) * 30,
    params.eyeWidth,
    30
  );
  pop();
...
//the following 2 easeOut functions are probably used to smooth the animations but i don't really understand how the code works - jiaz
...
//frequency is determined by the mouse's horizontal position (mouseX) on the canvas, moving the mouse to the left edge of the canvas sets the frequency to 100 Hz, and moving it to the right edge sets it to 500 Hz - jiaz
//amplitude is determined by mouseY, max volume at the top of the canvas and min volume at the bottom of the canvas - jiaz
...
function draw() {
  ...
  if (mouseIsPressed) {
    //i think i remember using lerp in one of my previous sketches but i kind of forgot what it does, function used to find a value between two numbers based on a percentage. The lerp function is commonly used in animation and drawing operations to smoothly transition between values, value1: The first value. value2: The second value. amt: The amount to interpolate between the two values. This is a number between 0.0 and 1.0, where 0.0 gives value1, 1.0 gives value2, and 0.5 gives the midpoint between value1 and value2, depending on which edge of the canvas the mouse is being pressed, the face will also exaggerate towards that side - jiaz
    ...
    for (let x = -width / 2 - cir / 2; x < width / 2 + cir * 2; x += xSpan) {
      ...
      if (params.bg == "rect") {
        ...
        curveVertex(
          x,
          y +
            (sin(x / 100 + y + frameCount / (100 + sin(y / 50) * 50)) * cir) / 2
        );
      } else {
  ...
  if (params.mood == "sad") {
    ...
    circle(
      -0,
      0,
      100 + (100 * sin(_mouseX / 100 + frameCount / 100)) / 10 + 200 * boomRatio
    );
