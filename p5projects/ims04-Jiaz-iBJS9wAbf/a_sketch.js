// https://editor.p5js.org/jiaying0822/sketches/iBJS9wAbf
// ims04-Jiaz
...
function toggleFullScreen() {
    let fs = fullscreen();
    fullscreen(!fs); // Toggle the state
    adjustVideoResolution();
}

function adjustVideoResolution() {
    if (fullscreen()) {
        // Set larger dimensions when in fullscreen
        my.width = displayWidth;
        my.height = displayHeight;
    } else {
        // Set smaller dimensions when not in fullscreen
        my.width = my.vwidth;
        my.height = my.vheight;
    }
    resizeCanvas(my.width, my.height);
    // Optionally reset video dimensions here if necessary
}
